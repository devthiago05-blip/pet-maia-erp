...
export type QueueFilter = "all" | "overdue" | "today" | "upcoming";
...
export interface PatientClinicStatus {
...
export function ClinicPrintView({
  patients,
  returns,
  vaccines,
  tasks,
...
export function DailyVetDashboard({
  returns,
  vaccines,
  tasks,
  hospitalizations,
...
export function ClinicDocumentWorkspace({
  patients,
  professionalName,
  professionalCrmv,
  onDocumentSave,
...
export function WeeklyReturnsQueue({
  returns,
...
export function VaccineAlertsQueue({
  vaccines,
  onConfirmationChange,
}: {
  vaccines: VaccineQueueItem[];
...
export function differenceInDays(date: Date, baseDate: Date) {
...
export function filterQueueItems<T extends { daysDiff: number }>(
  items: T[],
  filter: QueueFilter,
) {
  if (filter === "overdue") {
