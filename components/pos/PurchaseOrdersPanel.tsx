...
import { MessageCircle, PackageCheck, Plus, Printer, Send, Trash2, X } from "lucide-react";
...
export function PurchaseOrdersPanel({ orders, products, suppliers, onCreate, onStatus, onReceive }: {
  ...
  function shareOnWhatsApp(order: PurchaseOrder) {
    ...
    if (!phone) { toast.error("Cadastre um WhatsApp válido para o fornecedor"); return; }
    ...
    const total = (order.purchase_order_items || []).reduce((sum, item) => sum + item.ordered_quantity * Number(item.unit_cost), 0);
    const message = [
  ...
  return <section className="space-y-4 rounded-2xl border bg-white p-4 sm:p-5">
    ...
    {printing&&<PurchaseOrderPrintPreview order={printing} onClose={()=>setPrinting(null)}/>}
  </section>;
}

function PurchaseOrderPrintPreview({order,onClose}:{order:PurchaseOrder;onClose:()=>void}) {
  const items=order.purchase_order_items||[];
  const total=items.reduce((sum,item)=>sum+item.ordered_quantity*Number(item.unit_cost),0);
  return <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 p-4 sm:items-center print:static print:block print:bg-white print:p-0">
    <section className="document-print-area max-h-[calc(100dvh-2rem)] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl sm:p-8 print:max-h-none print:max-w-none print:overflow-visible print:rounded-none print:p-8 print:shadow-none">
      <div className="flex items-start justify-between gap-4 border-b-2 border-slate-900 pb-5">
        <div><p className="text-sm font-bold uppercase tracking-[0.2em] text-[#8A0EEA]">Pet Maia</p><h1 className="mt-1 text-2xl font-black">Pedido de compra</h1><p className="text-sm text-slate-500">#{String(order.id).padStart(6,"0")}</p></div>
        <div className="text-right text-sm"><p className="font-bold">Emissão</p><p>{formatDate(order.created_at)}</p><p className="mt-2 font-bold">Status</p><p>{order.status}</p></div>
      </div>
      <div className="mt-5 grid gap-4 rounded-xl bg-slate-50 p-4 sm:grid-cols-2 print:grid-cols-2">
        <div><p className="text-xs font-bold uppercase text-slate-500">Fornecedor</p><p className="mt-1 font-bold">{order.suppliers?.nome||"-"}</p><p className="text-sm">{order.suppliers?.documento||"Documento não informado"}</p><p className="text-sm">{order.suppliers?.contato||order.suppliers?.telefone||"Contato não informado"}</p></div>
        <div><p className="text-xs font-bold uppercase text-slate-500">Entrega</p><p className="mt-1 font-bold">{order.expected_date?formatDate(order.expected_date):"Sem previsão definida"}</p>{order.suppliers?.email&&<p className="text-sm">{order.suppliers.email}</p>}</div>
      </div>
      <div className="mt-6 overflow-x-auto print:overflow-visible"><table className="w-full min-w-[620px] border-collapse text-sm print:min-w-0"><thead><tr className="border-y-2 border-slate-900 text-left"><th className="py-3">Produto</th><th className="py-3 text-center">Qtd.</th><th className="py-3 text-right">Unitário</th><th className="py-3 text-right">Subtotal</th></tr></thead><tbody>{items.map(item=><tr key={item.id} className="border-b"><td className="py-3 pr-3"><strong>{item.products?formatProductName(item.products as Product):`Produto #${item.product_id}`}</strong>{item.products?.sku&&<p className="text-xs text-slate-500">SKU {item.products.sku}</p>}</td><td className="py-3 text-center">{item.ordered_quantity}</td><td className="py-3 text-right">{formatCurrency(item.unit_cost)}</td><td className="py-3 text-right font-semibold">{formatCurrency(item.ordered_quantity*Number(item.unit_cost))}</td></tr>)}</tbody></table></div>
      <div className="mt-5 flex justify-end"><div className="w-full max-w-xs border-t-2 border-slate-900 pt-3 text-lg font-black"><div className="flex justify-between"><span>Total estimado</span><span>{formatCurrency(total)}</span></div></div></div>
      {order.notes&&<div className="mt-6 rounded-xl border p-4"><p className="text-xs font-bold uppercase text-slate-500">Observações</p><p className="mt-1 whitespace-pre-wrap text-sm">{order.notes}</p></div>}
      <div className="mt-10 grid grid-cols-2 gap-10 text-center text-xs"><div className="border-t border-slate-500 pt-2">Pet Maia</div><div className="border-t border-slate-500 pt-2">Fornecedor</div></div>
      <p className="mt-8 text-center text-xs text-slate-400">Documento gerado pelo ERP Pet Maia</p>
      <div className="mt-6 grid gap-3 sm:grid-cols-2 print:hidden"><button type="button" onClick={onClose} className="rounded-xl border py-3">Fechar</button><button type="button" onClick={()=>window.print()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#8A0EEA] py-3 font-semibold text-white"><Printer size={18}/>Imprimir ou salvar em PDF</button></div>
    </section>
  </div>;
}
