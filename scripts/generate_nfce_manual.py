...
def build(path):
    ...
    checks = ["Empresa: razão social, CNPJ, IE, endereço, UF e município conferidos", "Regime tributário e série confirmados", "Certificado A1 dentro da validade e senha testada", "CSC e CSC ID do ambiente correto", "Todos os produtos do teste com selo Fiscal completo", "Ambiente definido como Homologação", "Venda de teste com forma de pagamento e totais conferidos", "XML autorizado, arquivado e DANFE/QR Code validados"]
    for item in checks: story.append(p(f"[ ] {item}"))
    story += [p("Pasta separada dos XML", "H2x"), p("Cada XML autorizado será guardado em uma área privada diferente do certificado, usando a organização <b>homologacao ou producao / ano / mês / chave-de-acesso.xml</b>. O acesso será restrito ao backend administrativo e a chave de acesso terá 44 dígitos.")]
